...
- **`supergateway --stateless` respawns the python child per request** — module-level caches (e.g. a singleton API client) are no-op under this mode. Each call re-imports everything and drives ~50-90 MB of RSS growth per call on Python MCPs with heavy SDKs (gRPC, google-ads, GA4). Mitigations: (a) set `mem_limit` + `pids_limit` on the service from day one so blow-ups stay contained to the container with `restart: unless-stopped` recovery; (b) ship the MCP's tool returns as compact JSON (`json.dumps(..., separators=(',', ':'))`) so payloads fit; (c) keep default `limit`/`page_size` parameters small (≤10) and let callers raise them explicitly. Surfaced on google-ads + GA4.
- **DO NOT use `supergateway --stateful` with claude.ai** — it trips supergateway issue #126 ("Conflict: Only one SSE stream is allowed per session") on every claude.ai session because claude.ai opens both POST and GET SSE streams. Result: SIGTERM of the python child after ~1 tool call, every subsequent call returns the generic claude.ai "Error occurred during tool execution" with no log on the server. Confirmed empirically on GA4 on 2026-04-29 (see `project_supergateway_stateful_bug126` memory). Stay stateless until #126 is closed upstream, or migrate the MCP server to native FastMCP HTTP transport (drop supergateway entirely).
- **Service-account JSON injection (GA4 / GSC pattern)** — public packages and forks expect a file path in `GOOGLE_APPLICATION_CREDENTIALS`. We don't bake the JSON into the image. Pattern from `mcp/ga4/Dockerfile`: take a single-line base64 env var `<NAME>_SERVICE_ACCOUNT_JSON_B64`, decode in a custom `/entrypoint.sh` to `/app/sa.json` at container start, then `exec supergateway "$@"`. Set `ENV GOOGLE_APPLICATION_CREDENTIALS=/app/sa.json` in the Dockerfile. JSON never lives in the image layer.
