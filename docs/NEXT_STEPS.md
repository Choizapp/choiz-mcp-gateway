...
- [x] **kapso** — done 2026-04-29 late. First **remote-proxy** MCP: gateway-only proxy to `https://app.kapso.ai/mcp` with `X-API-Key` injection. NO container, NO Dockerfile, NO CI build job; only `gateway/src/index.ts` + `compose.yml` env + `.env`. See `project_remote_proxy_pattern` and `project_kapso_deployed_state` memories. **Split per-project 2026-05-22**: Kapso API keys are project-scoped, so `/mcp/kapso` (OTP) was retired and replaced with one slug per project — `kapso-choiz-sales`, `kapso-choiz-support`, `kapso-timeless-sales`, `kapso-timeless-support`. **Affiliates project added 2026-09-15**: `kapso-affiliates` (no brand prefix — the program is cross-brand). Route + `KAPSO_API_KEY_AFFILIATES` are wired; the slug only registers once the key is appended to the EC2 `.env`.
